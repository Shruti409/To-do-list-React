...
const Task = () => {
    ...
    const handleEditClick = (todo, idx) => {
        const promptValue = prompt("Make changes", todo);
        if (promptValue !== null) {
    ...
    const taskList = tasks.map((todo, idx) => {
        return <li key={idx}>
            {todo}
            <button id="edit" onClick={() => handleEditClick(todo, idx)}>Update</button>
            <button id="del" onClick={() => handleDeleteClick(idx)}>Remove</button>
            <button id="up" onClick={() => handleUpClick(idx)}> Up</button>
            <button id="down" onClick={() => handleDownClick(idx)}> Down</button>
        </li>
    })
    return (
        <div>
            ...
            <button id="tskbtn" onClick={submitHandler}>ADD-Todo</button>
            <ul>
                {taskList}
            </ul>
        </div>
